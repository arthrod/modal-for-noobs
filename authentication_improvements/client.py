...
from modal_for_noobs.exceptions import MissingConfigException, modal_for_noobsException
from modal.cli.utils import timestamp_to_local
from modal_proto import api_pb2
from modal_utils.async_utils import synchronizer
from typing import Optional

from modal_for_noobs_modal.constants import DEPLOYMENT_PREFIX, ENDPOINT_PREFIX, ENDPOINT_POSTFIX
...
class ModalClient:
    """
    A class to wrap a modal client and validate and parse required modal_for_noobs-modal configuration

    modal_for_noobs-modal expects the following environment variables to be set:
        ...
        for env_var in EXPECTED_ENV_VARs:
            if env_var not in os.environ:
                raise MissingConfigException(
                    f"modal_for_noobs-modal requires {env_var} to be set."
                )
    ...
    async def get_deployment(self, name: str) -> Optional[dict]:
        ...
        raise modal_for_noobsException(f"No deployment with name {name} found")
