"""🚀💚 Modal Examples Explorer - Browse and deploy Modal examples in real-time!
"""
...
def sync_load_python_files(folder_choice):
    ...
    print(f"🔄 sync_load_python_files called with: {folder_choice}")
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
            result = loop.run_until_complete(load_python_files(folder_choice))
            print(f"🎯 sync_load_python_files result: {type(result)}")
            return result
        finally:
            loop.close()
    except Exception as e:
